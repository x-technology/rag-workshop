const { documentSplittingDemo } = require("./02-documentSplitter");
const { getEmbedModel } = require("./03-documentStorageEmbeddingsOllama");
const { setupChromaDB, searchChromaDB } = require("./04-documentStorageChroma");

async function run() {
  const { tokenChunks } = await documentSplittingDemo(`One of the most important things I didn't understand about the world when I was a child is the degree to which the returns for performance are superlinear.
  ...
  const store = await setupChromaDB(tokenChunks, getEmbedModel());
  const result = await searchChromaDB(store, "How to grow exponentially?");
  console.log(result);
}
